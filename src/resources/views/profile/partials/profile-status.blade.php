...
<br><h3 class="ui dividing header">{{ $user->username }}'s Statuses</h3>
...
<div class="row">
    <div class="col-sm-8 col-lg-6">
        @if (!$statuses->count())
            <p>There is nothing in {{ $user->username }}'s statuses timeline yet.</p>
        @else
                        ...
                        @endforeach
                            ...
                            <form role="form" action="{{ route('status.reply', ['statusId' => $status->id]) }}" method="post"  id="Public-reply-form">
                                <div class="form-group{{ $errors->has("reply-{$status->id}") ? ' has-error' : '' }}">
                                    <div class="ui form">
                                        <div class="field">
                                            <textarea name="reply-{{ $status->id }}" class="form-control" rows="3" placeholder="Reply to your own status"></textarea>
